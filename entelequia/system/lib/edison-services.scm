                       ...
                       (let ((arm-uid 232071)
                         ...
                         (for-each
                          ...
                          '("/media/rips/raw"
                            ...
                            "/media/rips/movies"))
                         ;; arm.yaml is generated by the ARM container on first start.
                         ;; The arm-config-patch shepherd service (below) patches it with
                         ;; secrets (TMDB key, MakeMKV key) after sops-secrets has run.
...
;;;
;;; The ARM container generates its own arm.yaml with defaults on first start
;;; (overwriting any minimal file we write at activation time, before sops runs).
;;; This one-shot service runs after sops-secrets has decrypted the TMDB key
;;; and patches the relevant fields in place.  ARM reads arm.yaml fresh for each
;;; rip (podman exec python3 main.py is a new process), so no container restart
;;; is needed — the next disc insertion will use the patched config.
  ...
  (program-file "arm-config-patch"
    #~(begin
        ...
        (define (patch-yaml-key content key new-val)
          ...
          ;; Works regardless of the old value.
