...
def _tray_status(devpath, logger=logging):
    ...
    try:
        ...
        # In rootless podman sr* nodes are root-only stubs; assume disc present
        if re.search(r'sr\d', devpath):
            logger.warning(f"Cannot open '{devpath}': {err!s}; assuming DISC_OK")
            return CDS.DISC_OK.value
        raise err
    try:
        return fcntl.ioctl(disk_check, 0x5326, 0)
    except OSError as err:
        logger.warning(f"Failed to check status for '{devpath}': {err!s}")
        # In rootless podman the container devtmpfs creates sr* nodes as
        # regular-file stubs (not real block devices), so CDROM_DRIVE_STATUS
        # ioctl always fails.  Return DISC_OK so ARM can proceed to identify
        # and rip the disc via MakeMKV.
        if re.search(r'sr\d', devpath):
            return CDS.DISC_OK.value
        return None
    finally:
