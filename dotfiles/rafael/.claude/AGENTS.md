...
Canonical, tool-neutral instructions that apply in every project
directory, for any coding agent that reads `AGENTS.md` (Claude Code via
the `@`-import in `CLAUDE.md`; pi/Codex/etc. natively).  A
project-specific `AGENTS.md`/`CLAUDE.md` at the repo root overrides
these.  Per-silo files inside `~/pks/` layer on when working there.
