...
;; Move (or promote) a denote note across PKS silos via denotecli,
;; preserving the denote ID so backlinks survive.  After a successful
;; move the buffer follows the file to its new silo.
...
(defun my-pks--reopen-after-move (id silo-label)
  "After a denotecli move, kill the old buffer and visit ID in SILO-LABEL."
  (let* ((silo-dir (my-pks--silo-path silo-label))
         (matches (and silo-dir (file-directory-p silo-dir)
                       (directory-files
                        silo-dir t
                        (concat "^" (regexp-quote id) ".*\\.org\\'"))))
         (new-file (car matches))
         (old-buf (current-buffer)))
    (when new-file (find-file new-file))
    (when (and (not (eq old-buf (current-buffer)))
               (buffer-live-p old-buf))
      (let ((kill-buffer-query-functions nil))
        (kill-buffer old-buf)))))

(defun my-pks--move-current-to (silo &optional kw-vocab restrict-to-path)
  "Move the current denote note to SILO via denotecli, preserving its ID.
...
file lives under that path."
  (let ((file (buffer-file-name)))
    ...
    (let* ((id (denote-retrieve-filename-identifier file))
           (kw (completing-read-multiple
                (format "%s keywords (comma-separated, empty = keep): " silo)
                kw-vocab))
           (args (append (list "rename" id "--silo" silo "--keep-id")
                         (when kw
                           (list "--keywords" (string-join kw ","))))))
      (save-buffer)
      (with-temp-buffer
        (let ((status (apply #'call-process "denotecli" nil t nil args)))
          (unless (zerop status)
            (user-error "denotecli failed (%d): %s"
                        status (string-trim (buffer-string))))
          (message "denotecli: %s"
                   (if (> (buffer-size) 0)
                       (string-trim (buffer-string))
                     (format "moved %s → %s" id silo)))))
      (my-pks--reopen-after-move id silo))))
