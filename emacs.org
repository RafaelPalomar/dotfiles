...
Welcome to my Emacs configuration file! At its base, this file is largely inspired by the configuration of David Wilson at [[https://codeberg.org/daviwil/dotfiles]], however, as the configuration grew up, other references and even AI has been used.
...
To achieve a consistent and minimalist Emacs appearance that combines the simplicity and colors of /Nano/ with the font styles of *Doom Emacs*, we'll refine the appearance settings in this section.

** Theme

*** Base theme

We use =entelequia-graphite=, the Emacs side of the
[[file:~/src/entelequia-theme/][entelequia-theme]] (PKS
=20260507T172436=).  It is built on =doom-themes='s
=def-doom-theme= macro — Dracula's broad mode coverage with
the Graphite & Vellum palette substituted (warm graphite
background, vellum cream foreground, oxidised copper accent).

The theme file lives at =~/.config/emacs/themes/entelequia-graphite-theme.el=
and is deployed by Guix Home from
=~/.dotfiles/dotfiles/.config/emacs/themes/=.

To switch back to upstream =doom-dracula= temporarily,
=M-x load-theme RET doom-dracula RET=.  To switch back
durably, replace =entelequia-graphite= with =doom-dracula=
in the =load-theme= call below.
...
Set up fonts to match Doom Emacs's style, using =JetBrains Mono= for fixed-pitch (monospace) text and =Cantarell= for variable-pitch text.
...
Load all-the-icons for better visuals in modes like Dired.
...
To bring the powerful and efficient modal editing experience of Vim to Emacs, we'll enable *Evil Mode*. Additionally, we'll incorporate several enhancements inspired by /Doom Emacs/ to provide a seamless and feature-rich editing environment.
...
We exclude dired since dirvish provides its own keybindings.
...
Evil Surround allows you to easily modify surrounding characters like brackets, quotes, etc.
...
Evil Matchit allows you to jump between matching tags, parentheses, braces, etc., using the =%= key.
...
Undo-Fu provides an intuitive and reliable undo/redo system.
...
To enhance our Org Mode experience with a modern look and conveniences similar to Doom Emacs, we'll configure several packages and settings.
...
=org-modern= is a package that revamps the look of Org documents.
...
=org-appear= makes hidden emphasis markers visible when the cursor is over them.
...
Enable the execution of code blocks in various languages.
...
** Setting Up org-babel LaTeX SVG export

Use =pdftocairo= (from poppler) to convert LaTeX-compiled PDFs to SVG when a
=latex= source block specifies a =.svg= output file.

#+begin_src emacs-lisp :tangle dotfiles/.config/emacs/init.el
(setq org-babel-latex-pdf-svg-process "pdftocairo -svg %f %o")
#+end_src

** Setting Up AUCTeX XeLaTeX engine

AUCTeX is installed but its autoloads don't claim =*.tex= by default — without
explicit activation, =.tex= files open in Emacs's built-in =latex-mode= and the
Command menu has no engine selector.  =tex-site= installs the AUCTeX mode map
(so =.tex= → AUCTeX's =LaTeX-mode=).  Then default the engine to XeTeX (=C-c C-c
LaTeX= runs =xelatex=) and register an explicit =XeLaTeX= entry in the Command
menu so it can be picked per-buffer even if the default is later flipped back
to pdfTeX.
...
** Custom Frame Helpers for Floating Windows

These helper functions make special Emacs frames (capture, dired, agenda) appear as floating, centered windows in bspwm.
...
Set the path to =mmdc= and configure sensible defaults so that mermaid blocks
only need to specify =:file=. With these defaults, blocks render to files on
evaluation and are embedded into HTML/LaTeX exports automatically.
...
Example: a block like the following renders =flow.svg= next to the org file
and embeds it on export.
...
Enable org-protocol for external capture via URLs.
...
Configure org-clock for time tracking with notifications and persistence.
...
Enhanced agenda views with automatic grouping.
...
** Org Columns (Kanban View)

Use org-columns for a kanban-like board view (built-in).
...
To enhance our Emacs experience with a powerful completion system similar to the one employed in Doom Emacs, we'll set up a completion framework using *=ivy=*, *=counsel=*, and *=swiper=*.
...
*=swiper=* offers an =ivy=-powered alternative to =isearch= for in-buffer searching.
...
*=ivy-rich=* provides additional information in =ivy= completion lists.
...
To bring the convenience of Doom Emacs's keybinding system into our configuration, we'll use the =general.el= package.
...
Now we'll define keybindings that correspond to the functionalities we've enabled.
...
[Perspective](https://github.com/nex3/perspective-el) provides tagged workspaces in Emacs.
...
This setup is inspired by [[https://github.com/rougier/dotemacs/blob/b31f0bea9ff8d6aa5c8e424bd4850bbc9b79470b/dotemacs.org#setup][rougier/dotemacs]].
...
** Notmuch (machine-queryable layer alongside mu4e)

Notmuch shares the Maildir with mu (mu4e). mu4e remains the primary
human UI; notmuch is the query layer used by scripts, shell one-liners,
and Claude skills via =notmuch search/show --format=json=. Installing
~emacs-notmuch~ also gives us a second-opinion UI that's sometimes more
pleasant for full-text search across large archives.
...
Dirvish provides a modern file browsing experience with Miller columns, file previews, and rich UI.
We exclude dired from evil-collection and define Evil-aware keybindings for dirvish.
...
Function-based Zettelkasten PKS under =~/pks/= (symlinked into
=~/Nextcloud/PKS/=).  Five active silos plus a =review-queue= for the
gradual promotion of legacy notes from =~/Notes/Personal/=.  The
=fleeting/= silo is the default =denote-directory= so bare =C-c n c=
lands there.  See plan:
...
Bridges Denote literature notes to a central bibliography at
=~/pks/library/references.bib=.  PDFs live under =~/pks/library/papers/=
named by citekey.  =citar-denote= creates literature notes with the
proper denote filename plus =#+reference: @citekey= front-matter.
...
** PDF reading and annotation — pdf-tools + org-noter

Renders PDFs in Emacs (=pdf-tools=), pins org notes to exact PDF
positions (=org-noter= + =org-pdftools=), and remembers per-PDF
reading position (=saveplace-pdf-view=).  Annotation workflow on a
literature note:

1. Add =#+PROPERTY: NOTER_DOCUMENT <path-to-pdf>= near the top of
   the note (file-level), or attach the property to a heading for a
   subtree-scoped binding.
2. =M-x org-noter= splits the window: PDF on one side, note on the
   other.
3. In the PDF buffer, select a region and =C-c C-a h= adds a
   highlight (saved into the PDF itself, portable to other readers).
4. =M-i= from the PDF buffer inserts a heading in the note pinned to
   the current page; type your reflection there.  Following that
   heading's link later returns to the exact PDF position.

Highlights live in the PDF, reflective notes live in the lit note —
each surface holds the kind of content it is best at.
